...
import os
import mlog
from train_muti_label import get_split, load_batch
import numpy as np
...
def run():
    ...
    with tf.Graph().as_default() as graph:
        ...
        images, raw_images, labels = load_batch(dataset, batch_size=FLAGS.batch_size, is_training = False, num_epochs=1)
        ...
        loss = tf.losses.log_loss(one_hot_label, sigmoid_op)
        ...
        def eval_step(sess):
            ...
            global_step_count, accuracy_value, step_logits, step_prediction, step_acc, step_loss, pred, label = sess.run([global_step_op, accuracy,
                                                                                                                logits_op, pred_op, acc_mine, loss, sigmoid_op, labels])
            time_elapsed = time.time() - start_time

            #Log some information
            # logging.info('Global Step %s: Streaming Accuracy: %.4f (%.2f sec/step)', global_step_count, accuracy_value, time_elapsed)
            logging.info('The averange accuracy of this batch(total 36 samples) is: %s, run time is:%s' % (step_acc, time_elapsed))
        ...
        with sv.managed_session() as sess:
            for step in xrange(num_steps_per_epoch * FLAGS.num_epochs):
                ...
                logging.info('Step #%d, Current Streaming Accuracy: %.4f', step, accuracy_value)
            ...
            plot_roc(total_pred, total_label, FLAGS.auc_picture_path)
