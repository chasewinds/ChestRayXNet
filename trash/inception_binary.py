...
def run():
    ...
    with tf.Graph().as_default() as graph:
        ...
        def load_batch_from_tfrecord(split_name, dataset_dir=FLAGS.tfrecord_dir, num_classes=FLAGS.num_classes,
            ...
            images, _, labels = load_batch(dataset, batch_size, num_classes, height=image_size, width=image_size, is_training=is_training, one_hot=True)
        ...
        with slim.arg_scope(inception_resnet_v2_arg_scope()):
            logits, end_points = inception_resnet_v2(train_images, num_classes=FLAGS.num_classes * 2, is_training=True)
        ## convert into probabilities
        # tf.Print(logits, ["logits: ", logits])
        logging.info('from logging, the shape of logits is : %s' % logits.get_shape())
        # prob = tf.sigmoid(logits)
        # tf.Print(probabilities, ["probabilities: ", probabilities])
        def get_prob(logits):
            prob_list = []
            for i in range(FLAGS.num_classes):
                sub_logits = tf.slice(logits, [0, 2*i], [FLAGS.batch_size, 2])
                # sub_prob = tf.nn.softmax(sub_logits)
                logging.info("the shape of sub_prob is : %s" % logits.get_shape())
                prob_list.append(sub_logits)
            prob = tf.stack(prob_list, axis=1)
            return prob

        logits = get_prob(logits)
        logging.info("the shape of prob after get_prob function is : %s" % logits.get_shape())
        ...
        lesion_pred = tf.cast(tf.greater_equal(logits, 0.5), tf.float32)
        ...
        with slim.arg_scope(inception_resnet_v2_arg_scope()):
            val_logits, val_end_points = inception_resnet_v2(val_images, num_classes=FLAGS.num_classes * 2, is_training=False, reuse=True)
        val_probabilities = get_prob(val_logits)

        ## new loss, just equal to the sum of 14 log loss
        val_loss = tf.nn.softmax_cross_entropy_with_logits(labels=val_labels, logits=logits)
        ...
        with sv.managed_session() as sess:
            ...
            for step in xrange(num_batches_per_epoch * FLAGS.num_epoch):
                ...
                if step % num_batches_per_epoch == 0:
                    ...
                    for i in xrange(int(val_num_batches_per_epoch / 5)): ## remember modify here!
