...
from chexnet import DenseNet
#from data_providers.utils import get_data_provider_by_name
from dataset_provider import DataProvider
import logging

train_params_chexnet = {
    'batch_size': 4,
    'n_epochs': 300,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--train_tfname', type=str, help='What train dataset should be used', required=True)
    parser.add_argument(
        '--validate_tfname', type=str, help='What valiation dataset should be used', required=True)
    parser.add_argument(
    ...
    # parser.add_argument(
    #     '--class_id', type=float, default=-1, metavar='',
    #     help='choies which kinds of disease we train our network to classify,if set to -1 means we need to classify all 14 disease.')
    # parser.add_argument(
    #     '--batch_size', type=int, default=4, metavar='',
    #     help='how many images in a single mini batch,')
    ...
    train_params = get_train_params_by_name("chexnet")
    print("Params:")
    ...
    data_provider = DataProvider(model_params['train_tfname'], 4, model_params['validate_tfname'], 4, class_id=3)
    # data_provider = DataProvider(model_params['train_tfname'], model_params['batch_size'], model_params['validate_tfname'], model_params['batch_size'], model_params['class_id'])
    print("Initialize the model..")
    # raw_images, label = data_provider.load_batch(data_provider,224,224,64)
    # logging.debug("label converted: %s" % label)
    logging.debug("model_params: %s" % model_params)

    model = DenseNet(data_provider=data_provider, **model_params)
    if args.train:
        print("Data provider train images: ", data_provider.train.n_samples)
        ## maybe load model here.
        # try:
        #     model.load_model()
        # except:
        #     pass
    ...
    #     if not args.train:
    #         model.load_model()
    #     print("Data provider test images: ", data_provider.test.n_samples)
    #     print("Testing...")
